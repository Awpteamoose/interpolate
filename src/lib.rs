...
pub fn s(input: TokenStream) -> TokenStream {
	...
	let fmt_tree = TokenTree::Literal(proc_macro::Literal::string(&fmt));
	let args_joined = args.into_iter().fold(TokenStream::new(), |mut stream, arg| {
		stream.extend(arg);
		let comma: TokenStream = TokenTree::Punct(proc_macro::Punct::new(',', proc_macro::Spacing::Alone)).into();
		stream.extend(comma);
		stream
	});
	let args_tree = TokenTree::Group(proc_macro::Group::new(proc_macro::Delimiter::None, args_joined));
	quote!({ format!($fmt_tree, $args_tree) })
}
...
pub fn p(input: TokenStream) -> TokenStream {
	...
	let args_tree = TokenTree::Group(proc_macro::Group::new(proc_macro::Delimiter::None, args_joined));
	quote!({ println!($fmt_tree, $args_tree) })
}
